...
class LSTM2AutoEncoder(AutoEncoder):
    ...
    def build_model(self, num_features, num_dims):
        ...
        self.encoder.add(layers.LSTM(48, activation='relu', input_shape=(1, num_features), return_sequences=True))
        self.encoder.add(layers.Dropout(rate=0.1))
        self.encoder.add(layers.LSTM(32, activation='relu', return_sequences=True))
        self.encoder.add(layers.Dropout(rate=0.1))
        ...
        self.decoder.add(layers.LSTM(32, activation='relu', return_sequences=True))
        self.encoder.add(layers.Dropout(rate=0.1))
        self.decoder.add(layers.LSTM(48, activation='relu', return_sequences=True))
        self.encoder.add(layers.Dropout(rate=0.1))
        self.decoder.add(layers.TimeDistributed(layers.Dense(num_features)))
