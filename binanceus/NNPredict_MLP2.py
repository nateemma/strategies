...
from NNPredictor_MLP2 import NNPredictor_MLP2

"""
####################################################################################
Predict_MLP2 - uses an MLP neural network to try and predict the future stock price
...
class NNPredict_MLP2(NNPredict):
    plot_config = {
        'main_plot': {
            'close': {'color': 'green'},
            # 'smooth': {'color': 'teal'},
    ...
    def get_classifier(self, pair, num_features: int, seq_len: int):
        return NNPredictor_MLP2(pair, seq_len, num_features)
