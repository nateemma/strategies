...
'''
####################################################################################
DWT_2 - use a Discreet Wavelet Transform (DWT) to estimate future price movements

This version 'just' looks for turnaround points
####################################################################################
'''


class DWT_2(IStrategy):
    ...
    buy_dwt_lookahead = IntParameter(0, 64, default=1, space='buy', load=True, optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative['dwt_predict'] = informative['close'].rolling(window=self.buy_dwt_window.value).apply(self.runDWT)
        ...
        dataframe['buy_region'] = np.where(qtpylib.crossed_above(dataframe['dwt_angle'], 0), 1, 0)
        dataframe['sell_region'] = np.where(qtpylib.crossed_below(dataframe['dwt_angle'], 0), 1, 0)
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # DWT triggers
        dwt_cond = (
            qtpylib.crossed_above(dataframe['dwt_angle'], 0.0)
        )
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # DWT triggers
        dwt_cond = (
            qtpylib.crossed_below(dataframe['dwt_angle'], 0.0)
        )
