...
'''
Classic MACD, using the macd and macsignal lines
'''
class MACDsignal(SimpleStrategy):
    ...
    plot_config = {
        ...
        'subplots': {
            "Diff": {
                'macd': {'color': 'lightseagreen'},
                'macdsignal': {'color': 'lightskyblue'},
            },
        }
    }

    enable_guards = False # set to True for testing, False for debug
    ...
    buy_params = {
        **SimpleStrategy.buy_params,
        ...
        "entry_signal_period": 16,
        "entry_slow_period": 47,
    }
    ...
    def get_entry_signals(self, dataframe):
        ...
        series = np.where(
            (
            # Only trigger when MACD crosses above the signal line
            # and both are meaningfully away from zero.
            qtpylib.crossed_above(dataframe['macd'], dataframe['macdsignal']) &
            (np.abs(dataframe['macd']) > 0.5) &
            (np.abs(dataframe['macdsignal']) > 0.5)
            ),
              1, 0)
    ...
    def get_exit_signals(self, dataframe):
        ...
        series = np.where(
            (
            # Symmetric exit: MACD crosses below the signal line
            # and both are meaningfully away from zero.
