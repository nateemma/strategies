...
'''
Classic TEMA (Zero Lag EMA) Crossing
'''
class TEMACross(SimpleStrategy):
    ...
    plot_config = {
        'main_plot': {
            ...
            'tema_short': {'color': 'lightseagreen'},
            'tema_long': {'color': 'lightsalmon'},
        },
        'subplots': {
            "Diff": {
            },
    ...
    def get_entry_signals(self, dataframe):
        ...
        if self.entry_short_period.value >= self.entry_long_period.value:
            dataframe['tema_short'] = 0.0
            dataframe['tema_long'] = 0.0
        else:
            dataframe['tema_short'] = fta.TEMA(dataframe, period=self.entry_short_period.value) # type: ignore
            dataframe['tema_long'] = fta.TEMA(dataframe, period=self.entry_long_period.value) # type: ignore

        series = np.where(
            (
                (dataframe['tema_short'] > dataframe['tema_long']) &
                (dataframe['close'] < dataframe['tema_short'])
            ),
            1, 0)
