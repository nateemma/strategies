...
for exchange in "${list[@]}"; do
  ...
  config_file="user_data/strategies/${exchange}/config_${exchange}.json"
    if [ "${exchange}" = "kucoin" ] || [ "${exchange}" = "ascendex" ]; then
        run_cmd "freqtrade download-data  -c ${config_file}  --timerange=${timerange} -t 1m 5m 15m 1h 1d"
        run_cmd "freqtrade download-data  -c ${config_file}  --timerange=${timerange} -t 1m 5m 15m 1h 1d -p BTC/USD BTC/USDT"
    else
