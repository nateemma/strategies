...
Checks the lookahead bias, based on backtest results
...
cmd="TF_FORCE_GPU_ALLOW_GROWTH=false freqtrade lookahead-analysis -vv --timerange=${timerange} -c ${config_file} --strategy-path ${spath} --strategy ${strategy}"
echo ${cmd}
eval ${cmd}
