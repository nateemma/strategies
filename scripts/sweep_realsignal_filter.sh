#!/bin/zsh
# Sweep gan_synth_realsignal_reject_pct on NNNC_DDPM_MLX @ ratio=0.4
# using the per-class binary signal classifiers trained by
# CreateRealsignalDiscriminator. Same baseline as the prior filter
# sweeps so the comparison is direct.
...
cleanup() {
  ...
  if [[ $INSERTED_RS_LINE -eq 1 ]]; then
    sed -i '' '/^    gan_synth_realsignal_reject_pct = .*/d' "$DDPM_FILE"
  fi
}
trap cleanup EXIT INT TERM

# Insert the realsignal knob after the density knob (idempotent).
if ! grep -q "^    gan_synth_realsignal_reject_pct = " "$DDPM_FILE"; then
  sed -i '' "/^    gan_synth_density_reject_pct = /a\\
    gan_synth_realsignal_reject_pct = 0.0
" "$DDPM_FILE"
  INSERTED_RS_LINE=1
fi
...
run_one() {
  ...
  echo ""
  echo "==================================================================="
  echo "[$(date +%H:%M:%S)] NNNC_DDPM_MLX @ ratio=${DDPM_SWEEP_RATIO} realsignal_reject_pct=${rej}"
  echo "==================================================================="
  ...
  sed -i '' "s/^    gan_synth_realsignal_reject_pct = .*/    gan_synth_realsignal_reject_pct = ${rej}/" "$DDPM_FILE"
  grep -E "    gan_target_ratio|    gan_synth_(density|discrim|neural_discrim|realsignal)_reject_pct" "$DDPM_FILE"

  rm -rf "${ROOT}/user_data/strategies/saved_data/NNNC_DDPM_MLX"

  local log="${LOG_DIR}/NNNC_DDPM_MLX_r${DDPM_SWEEP_RATIO}_rs${rej}.log"
...
echo "[$(date +%H:%M:%S)] realsignal sweep started"
