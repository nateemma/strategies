...
# runs recursive-analysis on a single strategy.
#
# Use this instead of check_bias.sh for hold-and-rebalance strategies (e.g.
# the Basket family) that have no sell signals: lookahead-analysis needs
# completed entry->exit trades to compare, so it can't analyse a strategy
# that never exits. recursive-analysis checks indicator causality directly
# (recomputes each indicator with increasing startup candles and flags any
# candle whose value changes), which needs no trades at all.
...
Checks for recursive-formula / indicator lookahead bias (freqtrade
recursive-analysis). Suitable for strategies with no sell signals, where
lookahead-analysis cannot run.
...
<group>  Either subgroup (e.g. Basket) or name of exchange (binanceus, coinbasepro, kucoin, etc)
...
today=`date`
echo $today
echo "Recursive-analysis of strategy:$strategy for exchange:$exchange..."

spath="${group_dir}"

cmd="TF_FORCE_GPU_ALLOW_GROWTH=false freqtrade recursive-analysis --timerange=${timerange} -c ${config_file} --strategy-path ${spath} --strategy ${strategy}"
echo ${cmd}
eval ${cmd}
