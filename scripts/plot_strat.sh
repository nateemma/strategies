...
[options]:  -c | --config      path to config file (default: user_data/strategies/<group>/config_<group>.json
            ...
            --no-trades        Plot without trades data (just indicators)
            --backtest-file    Specify backtest results file to use for plotting
...
while getopts :c:ln:st:-: OPT; do
  ...
  case "$OPT" in
    ...
    --no-trades )    no_trades=1 ;;
    --backtest-file ) needs_arg; backtest_file="$OPTARG" ;;
...
today=`date`
echo $today
echo "Plotting strategy:$strategy for group:$group, timeframe:${timerange}"

# Build the plotting command
cmd="freqtrade plot-dataframe --timerange=${timerange} -c ${config_file} --strategy-path ${group_dir} -p ${pair} -s ${strategy}"

# Add backtest file or no-trades option
if [[ $no_trades -eq 1 ]]; then
    cmd="${cmd} --no-trades"
    echo "Using --no-trades mode (plotting indicators only)"
elif [[ -n "${backtest_file}" ]]; then
    cmd="${cmd} --backtest-filename ${backtest_file}"
    echo "Using specified backtest file: ${backtest_file}"
else
    # Try to find the most recent backtest result file
    backtest_results_dir="user_data/backtest_results"
    if [[ -d "${backtest_results_dir}" ]]; then
        latest_file=$(ls -t ${backtest_results_dir}/backtest-result-*.json 2>/dev/null | head -1)
        if [[ -n "${latest_file}" ]]; then
            # Extract just the filename, not the full path
            filename=$(basename "${latest_file}")
            cmd="${cmd} --backtest-filename ${filename}"
            echo "Using most recent backtest file: ${filename}"
        else
            echo "No backtest result files found. Using --no-trades mode."
            cmd="${cmd} --no-trades"
        fi
    else
        echo "Backtest results directory not found. Using --no-trades mode."
        cmd="${cmd} --no-trades"
    fi
fi
