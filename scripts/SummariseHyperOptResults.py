...
def process_totals(strat, line):
    ...
    cols = " ".join(
        line[1:].split()
    )  # get rid of multiple spaces (and 1st character, which could be '*')
    ...
    if len(strat_summary) > 0:
        ...
        for strategy in strat_summary:
            strat_stats.append(
                [
                    strategy,
                    strat_summary[strategy]["entries"],
                    strat_summary[strategy]["ave_profit"],
                    strat_summary[strategy]["tot_profit"],
                    strat_summary[strategy]["win_pct"],
                    0.0,
                ]
            )
