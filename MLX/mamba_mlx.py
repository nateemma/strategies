...
from MLX.pscan_mlx import pscan
...
class MambaConfig:
    ...
    bias: bool = False
    conv_bias: bool = True
    d_inner: int = 0  # set in __post_init__

    pscan: bool = (
        False  # use parallel scan mode or sequential mode when training. on MLX, the pscan isn't performant.
    )
...
class MambaBlock(nn.Module):
    def __init__(self, config: MambaConfig):
        ...
        if config.dt_init == "constant":
            ...
            self.dt_proj.weight = mx.random.uniform(
                -dt_init_std, dt_init_std, self.dt_proj.weight.shape
            )
        else:
        ...
        dt = mx.clip(
            mx.exp(
                mx.random.uniform(shape=[config.d_inner])
                * (math.log(config.dt_max) - math.log(config.dt_min))
                + math.log(config.dt_min)
            ),
            a_min=config.dt_init_floor,
            a_max=None,
        )
        inv_dt = dt + mx.log1p(
            -mx.exp(-dt)
        ...
        A = mx.repeat(
            mx.arange(1.0, 16 + 1.0).reshape([1, 16]), repeats=config.d_inner, axis=0
        )
        self.A_log = mx.log(
            A
    ...
    def selective_scan(self, x, delta, A, B, C, D):
        ...
        y = (hs @ mx.expand_dims(C, -1)).squeeze(
            3
    ...
    def selective_scan_seq(self, x, delta, A, B, C, D):
        ...
        h = mx.zeros(
            [x.shape[0], self.config.d_inner, self.config.d_state]
        )  # (B, ED, N)
        hs = []
    ...
    def ssm_step(self, x, h):
        ...
        A = -mx.exp(
            self.A_log
        )  # (ED, N) # todo : move out of step (timestep independent)
        D = self.D
