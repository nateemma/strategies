...
from MLX.mamba_mlx import Mamba, MambaConfig
from MLX.mlx_misc import topk
from MLX.mlx_utils import load_config_hf, load_state_dict_hf
...
class MambaLM(nn.Module):
    def __init__(self, lm_config: MambaLMConfig):
        ...
        )  # TODO this does not really tie the weights, investigate
    ...
    def generate(
        ...
        input_ids = mx.array(
            tokenizer(prompt, return_tensors="np").input_ids
        )  # (1, tokens_prompt) # (1, num_tokens)
        ...
        caches = [
            (None, mx.zeros([1, self.config.d_conv - 1, self.config.d_inner]))
        ...
        for i in range(input_ids.shape[1] + n_tokens_to_gen - 1):
            ...
            if i + 1 >= input_ids.shape[1]:
                ...
                if top_k is not None:
                    ...
                    )  # (1, k) ordered from lowest to biggest
                    mask = next_token_logits < (values[:, 0, None])
                    next_token_logits = mx.where(
                        mask, -5000, next_token_logits
                    )  # TODO -mx.inf is problematic for now

                if sample and temperature > 0:
                    next_token = mx.random.categorical(
                        next_token_logits * (1 / temperature), num_samples=1
                    )
                else:
