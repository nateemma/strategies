...
class KerasClassifierEncoder(KerasBaseClassifier):
    ...
    def create_model(self, seq_len, num_features):
        ...
        model.add(
            layers.Dense(inner_dim, activation="relu", name=self.encoder_layer)
        )  # name is mandatory

        # Decoder
        model.add(
            layers.Dense(2 * outer_dim, activation="relu", input_shape=(1, inner_dim))
        )
