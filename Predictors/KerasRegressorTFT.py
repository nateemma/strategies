# A subclass of KerasRegressor that provides a Classifier for use with Google's Temporal Fusion Transformer (TFT)
...
class KerasRegressorTFT(KerasRegressor):
    ...
    def create_tft_model(self, dataframe: DataFrame):
        ...
        self.tft_params["input_obs_loc"] = [
            dataframe.columns.get_loc(c) for c in tgt_cols if c in dataframe
        ]
        self.tft_params["static_input_loc"] = [
            dataframe.columns.get_loc(c) for c in static_cols if c in dataframe
        ]
        ...
        self.tft_params["category_counts"] = json.dumps(
            [
                len(tgt_cols),
                len(observed_cols),
                len(known_cols),
        ...
        print(
            f"self.tft_params['category_counts']: {self.tft_params['category_counts']}"
        )
        ...
        self.tft_params["total_time_steps"] = (
            self.tft_params["num_encoder_steps"] + self.seq_len
        )
    ...
    def train(
        ...
        super(KerasRegressorTFT, self).train(
            df_train_norm, df_test_norm, train_results, test_results, force_train
        )
