...
class NNPredict(IStrategy):
    plot_config = {
        ...
        'subplots': {
            "Diff": {
                ...
                'trend': {'color': 'lightgreen'},
                'raw_gain': {'color': 'lightpink'},
    ...
    force_normalise = False # overrides predictor setting, if needed
    ...
    seq_len = 1  # 'depth' of training sequence
    ...
    predictor_type = NNPredictors.PredictorType.LSTM # override in subclass to change model type
    ...
    dbg_test_predictor = True  # test clasifiers after fitting
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if NNPredict.first_time:
            ...
            print(f"    Combine models: {self.combine_models}")
    ...
    def add_indicators(self, dataframe: DataFrame) -> DataFrame:
        ...
        dataframe = self.dataframePopulator.add_indicators(dataframe, dataset_type=self.dataset_type) # type: ignore

        # TEST: use smooth trend instead of real gain
        # detrender = make_detrender(DetrenderType.EMA)
        # detrender = make_detrender(DetrenderType.DWT)
        ...
        dataframe['raw_gain'] = dataframe['gain']
        dataframe['gain'] = detrend
    ...
    def train_model(self, dataframe: DataFrame, pair) -> DataFrame:
        ...
        df = dataframe.copy()
        df = self.dataframePopulator.drop_index(df) # type: ignore
        ...
        if self.model_per_pair:
            if self.curr_pair not in self.classifier_list:
                self.classifier_list[self.curr_pair] = self.make_predictor(self.curr_pair, self.seq_len, nfeatures)
            self.curr_predictor = self.classifier_list[self.curr_pair]
        ...
        if self.prescale_required:
            df_norm = self.dataframeUtils.norm_dataframe(df) # type: ignore
        else:
            df_norm = df.copy()
        ...
        dataframe['%future_gain'] = future_gain # note: dataframe, not df
        target = future_gain
        ...
        if self.curr_predictor.needs_dataframes():
            ...
            df_tensor = self.dataframeUtils.df_to_tensor(df_norm, self.seq_len) # type: ignore
    ...
    def backtest_data(self, dataframe: DataFrame) -> DataFrame:
        ...
        classifier = self.curr_predictor
        use_dataframes = classifier.needs_dataframes() # type: ignore
        ...
        if self.prescale_required:
            ...
            df_norm = self.dataframeUtils.norm_dataframe(dataframe) # type: ignore
        else:
            df_norm = dataframe
        ...
        if use_dataframes:
            ...
            data = self.dataframeUtils.df_to_tensor(df_norm, self.seq_len) # type: ignore

        # run backtest
        preds_notrend = classifier.backtest(data) # type: ignore
    ...
    def update_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        if use_dataframes:
            ...
            tensor = self.dataframeUtils.df_to_tensor(df_norm, self.seq_len) # type: ignore
        ...
        if classifier.returns_single_prediction(): # type: ignore
            predictions = classifier.predict(data) # type: ignore
    ...
    def add_model_batch_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        while end < nrows:
            ...
            if self.prescale_required:
                predictions = self.denorm_array(preds)
            else:
                predictions = preds
    ...
    def add_predictions(self, dataframe: DataFrame, pair) -> DataFrame:
        ...
        df = dataframe.copy()
        df = self.dataframeUtils.remove_debug_columns(df) # type: ignore
        df = self.dataframePopulator.drop_index(df) # type: ignore

        # print(f'columns: {df.columns}')

        df = self.add_model_predictions(df)
        # df = self.update_predictions(df)
        # dataframe['predict_smooth'] = dataframe['predicted_gain'].rolling(window=win_size).apply(self.roll_strong_smooth)
        dataframe['predicted_gain'] = df['predicted_gain'].clip(lower=-3.0, upper=3.0)
    ...
    def make_predictor(self, pair, seq_len: int, num_features: int):
    ...
    def custom_exit(
        ...
        if trade.is_short:
            ...
            if current_profit > 0.0:
                ...
                if last_candle["predicted_gain"] <= last_candle["target_loss"]: # type: ignore
                    return "predict_drop"
