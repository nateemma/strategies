...
"""
####################################################################################
NNGain - uses a Long-Short Term Memory neural network to try and predict the future stock price
      
      This works by creating a LSTM model that we train on the historical gain, then use that model to predict 
      future values

      Note that this uses *only* the gain history, no other parameters

####################################################################################
"""


class NNGain(IStrategy):
    plot_config = {
        'main_plot': {
            'close': {'color': 'lightsteelblue'},
            'bb_middleband': {'color': 'lightgreen'},
        },
        'subplots': {
            "Diff": {
    ...
    curr_dataframe: DataFrame = None # type: ignore
    prescale_required = False # set by predictor
    force_normalise = True # overrides predictor setting, if needed
    ...
    seq_len = 1  # 'depth' of training sequence (use bigger than normal value since we are only using gain)
    num_epochs = 128  # max number of iterations for training
    batch_size = 1024  # batch size for training
    predict_batch_size = 128

    predictor_list = {}  # predictor for each pair
    curr_predictor = None
    ...
    dataframeUtils: DataframeUtils = None  # type: ignore
    dataframePopulator: DataframePopulator = None  # type: ignore
    ...
    dbg_test_predictor = False  # test predictors after fitting
    dbg_curr_df: DataFrame = None  # type: ignore # for debugging of current dataframe
    ...
    enable_bb_check = CategoricalParameter([True, False], default=False, space="buy", load=True, optimize=False)

    enable_squeeze = CategoricalParameter([True, False], default=False, space="buy", load=True, optimize=False)

    entry_guard_metric = DecimalParameter(-0.8, -0.2, default=-0.2, decimals=1, space="buy", load=True, optimize=True)

    entry_bb_width = DecimalParameter(0.020, 0.100, default=0.02, decimals=3, space="buy", load=True, optimize=False)

    entry_bb_factor = DecimalParameter(0.70, 1.20, default=1.1, decimals=2, space="buy", load=True, optimize=False)
    ...
    exit_bb_factor = DecimalParameter(0.70, 1.20, default=0.8, decimals=2, space="sell", load=True, optimize=False)
    ...
    cexit_min_loss_th = DecimalParameter(-1.5, -0.5, default=-0.5, decimals=1, space="sell", load=True, optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if NNGain.first_time:
            NNGain.first_time = False
        ...
        # update info for use in custom_exit etc
    ...
    # dataset_type = DatasetType.GAIN_ONLY # can change this in subclass if desired
    dataset_type = DatasetType.MINIMAL # can change this in subclass if desired
    ...
    def reduce_dataframe(self, dataframe: DataFrame) -> DataFrame:
        ...
        df = dataframe[['gain', 'open', 'close', 'high', 'low', 'volume']].copy()
        ...
        detrend = np.round(detrend, decimals=3)
        df['gain'] = detrend

        # # Hack: constrain to 3 decimal places to make it a little easier for the model
        # df['gain'] = np.round(df['gain'], decimals=3)
    ...
    def train_model(self, dataframe: DataFrame, pair) -> DataFrame:
        ...
        # only use 'gain'
        df = self.reduce_dataframe(dataframe)

        # DEBUG:
        # print(f'Columns: {df.columns}')

        nfeatures = np.shape(df)[-1]

        # create the predictor if it doesn't already exist
        if self.model_per_pair:
            if self.curr_pair not in self.predictor_list:
                self.predictor_list[self.curr_pair] = self.make_predictor(self.curr_pair, self.seq_len, nfeatures)
            self.curr_predictor = self.predictor_list[self.curr_pair]

        else:
            if not self.curr_predictor:
                self.curr_predictor = self.make_predictor(self.curr_pair, self.seq_len, nfeatures)

        # check to see of model is already trained (and not in force training mode). If so, save time and return
        force_train = self.refit_model if (self.dp.runmode.value in ('backtest')) else False
        if self.curr_predictor.model_is_trained() and (not force_train) :
            return dataframe
        ...
        # scale the dataframe
        if self.prescale_required:
            tmp = self.norm_array(df[self.target_column].to_numpy()) # prep for inverse transform
            df_norm = self.dataframeUtils.norm_dataframe(df)
        else:
            df_norm = df.copy()
            # df_norm = self.dataframeUtils.convert_index(df)

        # print("")
        # print("    df:     ", np.shape(df))
        # print("    df_norm:", np.shape(df_norm))
        # print("")

        # future_gain = df_norm['gain'].shift(-self.lookahead)
        future_gain = df['gain'].shift(-self.lookahead)
        future_gain = np.nan_to_num(future_gain)
        future_gain = np.round(future_gain, decimals=2) # round so that prediction is easier
        df['%future_gain'] = future_gain
        target = future_gain
        self.curr_predictor.set_target_column('gain')
        ...
        # some predictors take DataFrames as input, others tensors, so check
        if self.curr_predictor.needs_dataframes():
        ...
        self.curr_predictor.train(train_data, test_data,
                                   train_results, test_results,
                                   force_train)

        if self.dbg_test_predictor:
            self.curr_predictor.evaluate(test_data, test_results)

        # If a new model, set training mode
        # print(f'    New model: {self.curr_predictor.new_model_created()}')
        if self.curr_predictor.new_model_created():
    ...
    array_scaler: ScalerType = None # type: ignore

    # scale the price data.
    def norm_array(self, arr: np.array) -> np.array: # type: ignore

        if (self.scaler_type != ScalerType.NoScaling) and self.scale_target:

            if self.array_scaler is None:
                self.array_scaler = self.dataframeUtils.make_scaler() # type: ignore

            if len(arr) > 0:
                narr = self.array_scaler.fit_transform(arr.reshape(-1, 1)) # type: ignore
    ...
    def denorm_array(self, arr: np.array) -> np.array: # type: ignore
        ...
        if (self.scaler_type != ScalerType.NoScaling) and self.scale_target:
            ...
            if len(arr) > 0:
                darr = self.array_scaler.inverse_transform(arr.reshape(-1, 1)) # type: ignore
    ...
    def backtest_data(self, dataframe: DataFrame) -> DataFrame:
        ...
        # get the current predictor and relevant flags
        predictor = self.curr_predictor
        use_dataframes = predictor.needs_dataframes() # type: ignore
        ...
        # run backtest
        preds_notrend = predictor.backtest(data) # type: ignore
        ...
        if self.prescale_required:
            ...
            # predictor handles scaling
            predictions = preds_notrend
        '''

        # # TEMP HACK: last value is suspicous, drop it!
        # predictions = predictions[:-1]

        plen = min(len(predictions), len(dataframe['predicted_gain']))
        dataframe['predicted_gain'][-plen:] = predictions[-plen:]
    ...
    def update_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        if predictor.returns_single_prediction(): # type: ignore
            predictions = predictor.predict(data) # type: ignore
    ...
    def add_model_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        # if self.curr_predictor.returns_single_prediction():
    ...
    def add_model_batch_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        if not use_dataframes:
            ...
            df_tensor = df_norm # won't run, just keep linter happy
        ...
        predictions: np.array = np.zeros(np.shape(dataframe['gain']), dtype=float) # type: ignore
    ...
    def add_model_rolling_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        # probably don't need to handle tensor cases since those predictors typically do not return single values
        ...
        if not use_dataframes:
            ...
            df_tensor = df_norm # just to keep the linter happy

        window = 64
        start = window
        end = np.shape(df_norm)[0]

        preds_notrend: np.array = [] # type: ignore
    ...
    def add_predictions(self, dataframe: DataFrame, pair) -> DataFrame:
        ...
        self.custom_trade_info[self.curr_pair]["curr_bb_mid"] = dataframe['bb_middleband'].iloc[-1]
    ...
    def make_predictor(self, pair, seq_len: int, num_features: int) -> NNPredictors.NNPredictor:
        predictor = self.get_predictor(pair, seq_len, num_features)
        ...
        self.prescale_required = (predictor.prescale_data()) or (self.force_normalise)

        return predictor

    # returns the predictor model. Override this function to change the type of predictor
    def get_predictor(self, pair, seq_len: int, num_features: int):
        # get the predictor type identified by self.predictor_type - override in definition in subclass to change type

        predictor = NNPredictors.make_predictor(self.predictor_type, pair, seq_len, num_features)
        return predictor
    ...
    def get_compressor(self, df_norm: DataFrame):
        # just use fixed size PCA (easier for predictors to deal with)
        ncols = int(64)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if guard_conditions:
            ...
            model_cond = (
                ...
                &
                (dataframe['close'] < dataframe["bb_middleband"]) # should change to rolling mid point of window
            )
        else:
            # model triggers
            model_cond = (
        ...
        strong_buy = (
            # predict >= 1% gain and below average price
            (
            (dataframe["predicted_gain"] >= self.cexit_min_profit_th.value)
            &
            qtpylib.crossed_above(dataframe["predicted_gain"], 1.0)
            # (dataframe["predicted_gain"] >= 1.0)
            &
            (dataframe["close"] <= dataframe["bb_middleband"])
            )
            # |
            # # predict gain >= 1.5% (no guard)
            # (
            # qtpylib.crossed_above(dataframe["predicted_gain"], 1.5)
            #  )
        )

        # conditions.append(metric_cond)
        conditions.append(model_cond | strong_buy)
        ...
        # For debug/visualisation (buy=1, sell=-1):
        dataframe['%model_signal'] = np.where((dataframe["predicted_gain"] > dataframe["target_profit"]), 
                                             1, 0)
        dataframe['%model_signal'] = np.where((dataframe["predicted_gain"] < dataframe["target_loss"]), 
                                             -1, dataframe['%model_signal'])
        ...
        # p_gain = float(dataframe['predicted_gain'].iloc[-1]) # type: ignore
        # t_gain = float(dataframe["target_profit"].iloc[-1]) # type: ignore
        # t_gain2 = float(dataframe["target_profit"].iloc[-2]) # type: ignore
        # if (p_gain > t_gain):
        #     print(f"{self.curr_pair} - potential buy. Gain: {p_gain:.4f}")

        # # DEBUG
        # print(f"p_gain:{dataframe['predicted_gain'].iloc[-8:]}")
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.enable_squeeze.value:
            if ("squeeze" not in dataframe.columns):
                dataframe["squeeze"] = np.where((dataframe["bb_width"] >= self.entry_bb_width.value), 1, 0)
        ...
        if conditions:
            dataframe.loc[reduce(lambda x, y: x & y, conditions), "exit_signal"] = 1 # used in custom_exit
            # only set exit_long column if exit signals are enabled
            if self.enable_exit_signal.value:
                dataframe["exit_long"] = dataframe["exit_signal"]

        return dataframe

    ###################################

    # double-check trade entry before committing
    def confirm_trade_entry(
        ...
        **kwargs,
        ...
        # Note: f-string formatting doesn't always work in this callback, so we use manually constructed strings instead
        ...
        if pair in self.custom_trade_info:
            ...
            # print(f"    *** Trade entry: {pair} custom_trade_info:{self.custom_trade_info[pair]}")

            cancel_str = "    *** " + pair + " Trade cancelled. "

            # check predicted gain

            curr_p_gain = self.custom_trade_info[pair]["curr_prediction"]
            if curr_p_gain <= 0.0:
                # print("")
                out_str = cancel_str + "Negative predicted gain! (ERROR)"
                print(out_str)
                print("")
                return False
            ...
            if rate >= curr_target:
                # print("")
                s_tgt = str(round(curr_target, 4))
                out_str = cancel_str + "Rate (" + s_rate + ") above target (" + s_tgt + "}"
                print(out_str)
                return False

            # check rate is still below mid bollinger band (when buy was signalled, not current candle)

            curr_bb_mid = self.custom_trade_info[pair]["curr_bb_mid"]
            if rate >= curr_bb_mid:
                s_bb = str(round(curr_bb_mid, 4))
                out_str = cancel_str + "Rate (" + s_rate + ") above BB mid (" + s_bb + "}"
                print(out_str)
                return False
        else:
            # we don't have data from when trade was tagged
            print(f"    WARN: no custom_trade_info for: {pair}")
            curr_pred = 0.0
            curr_target = 0.0

        # check that current rate is still below BB midband (in all cases)
        dataframe, _ = self.dp.get_analyzed_dataframe(pair=pair, timeframe=self.timeframe)
        last_candle = dataframe.iloc[-1].squeeze()
        curr_bb_mid = float(last_candle["bb_middleband"]) # type: ignore
        if rate >= curr_bb_mid:
            cancel_str = "    *** " + pair + " Trade cancelled. "
            out_str = cancel_str + "Rate (" + s_rate + ") above BB mid (" + str(round(curr_bb_mid, 4)) + "}"
            print(out_str)
            return False

        # check that current predicted gain is still > 0 (in all cases)
        dataframe, _ = self.dp.get_analyzed_dataframe(pair=pair, timeframe=self.timeframe)
        last_candle = dataframe.iloc[-1].squeeze()
        p_gain = float(last_candle['predicted_gain']) # type: ignore
        if p_gain <= 0.0:

            out_str = "    *** " + pair + " Trade cancelled. Predicted gain: " + str(round(p_gain, 4))
            print(out_str)
            return False
        ...
        p_str = (
            ...
            + " Predicted gain: " + str(round(curr_pred, 4))
            + "% Target: " + str(round(curr_target, 4))
        )
        # print(
        #     f"    Trade OK: {pair}, rate: {rate:.4f} Predicted gain: {curr_pred:.2f}% Target: {curr_target:.2f}%"
        # )
        print(p_str)
        print("")
    ...
    # double-check exit before committing
    def confirm_trade_exit(
        ...
        **kwargs,
        ...
        # print("*")
        # str = f'*    Trade Exit: {pair}, rate: {rate:.4f)} reason:{exit_reason}'
        ...
        # TEST: don't exit if predicted profit > 0.1%
        dataframe, _ = self.dp.get_analyzed_dataframe(pair=pair, timeframe=self.timeframe)
        last_candle = dataframe.iloc[-1].squeeze()
        p_gain = float(last_candle['predicted_gain']) # type: ignore
        if p_gain > 0.1:
            pstr = '*    Trade Exit Cancelled: gain still predicted'
            print(pstr, flush=True)
            # print("")
            return False
    ...
    def custom_exit(
        ...
        # # check volume?!
        # if last_candle["volume"] <= 1.0: # type: ignore
        #     return None
        ...
        if trade.is_short:
            ...
            if current_profit > 0.0:
                ...
                if last_candle["guard_metric"] >= self.cexit_metric_overbought.value: # type: ignore
                    return "metric_overbought"

                # Above 0.5%, sell if guard metric in sell range
                if current_profit > 0.005:
                    if last_candle["guard_metric"] >= self.cexit_metric_take_profit.value: # type: ignore
                        return "take_profit"

                # big drop predicted. Should also trigger an exit signal, but this might be quicker (and will likely be 'market' sell)
                pgain = float(last_candle["predicted_gain"]) # type: ignore
                tloss = float(last_candle["target_loss"]) # type: ignore
                if (pgain <= tloss) & (pgain <= self.cexit_min_loss_th.value):
                    # print(f'    predict_drop: predict gain: {pgain:.4f} Target loss: {tloss:.4f}')
                    return "predict_drop"

                # if in profit and exit signal is set, sell (even if exit signals are disabled)
                if last_candle["exit_signal"] > 0: # type: ignore
                    return "exit_signal"

                # if in profit and exit signal is set, sell (even if exit signals are disabled)
                if last_candle["%model_signal"] < 0: # type: ignore
                    return "model_signal"
                ...
                if pair in self.custom_trade_info:
                    if current_profit > 0.005:
                        curr_target = float(last_candle["curr_target"]) # type: ignore
                        curr_pred = float(last_candle["predicted_gain"]) # type: ignore
                        if (
                            (curr_target > 0.0)
        ...
        if (num_hours >= 12) & (current_profit > 0.002):
            return "unclog_12h"
