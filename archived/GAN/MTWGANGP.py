...
class MultiTaskWGANGP(GANBase):
    """Object-oriented wrapper around the multi-task WGAN-GP implementation."""

    MODEL_FILENAME = "mt_wgangp_model.weights.h5"
    ...
    def augment(
        ...
        labels: Dict[str, np.ndarray | list],
        *,
        task_target_ratios: Optional[Dict[str, Any]] = None,
        primary_task: str = "trading",
        **kwargs: Any,
    ) -> Tuple[pd.DataFrame, Dict[str, np.ndarray]]:
        extra = dict(kwargs)
        if task_target_ratios is not None:
            extra["task_target_ratios"] = task_target_ratios
        extra.setdefault("primary_task", primary_task)
    ...
    def _train_impl(
        ...
        encoded, _ = _encode_multi_labels(labels)
        balance_with_mt_wgan_gp(values, encoded, **kwargs)
    ...
    def _augment_impl(
        ...
        encoded, meta = _encode_multi_labels(labels)
        aug_values, aug_labels = balance_with_mt_wgan_gp(values, encoded, **kwargs)
        aug_df = pd.DataFrame(aug_values, columns=columns)
        restored = _decode_multi_labels(aug_labels, meta)
        return aug_df, restored

__all__ = ["MultiTaskWGANGP"]


def _encode_multi_labels(labels: Dict[str, np.ndarray | list]) -> Tuple[Dict[str, np.ndarray], Dict[str, np.dtype]]:
    if not isinstance(labels, dict):
        raise TypeError("Multi-task labels must be provided as a dictionary of 1D class index arrays")
    encoded: Dict[str, np.ndarray] = {}
    meta: Dict[str, np.dtype] = {}
    for task, value in labels.items():
        arr = np.asarray(value)
        if arr.ndim != 1:
            raise ValueError(f"Labels for task '{task}' must be 1D class indices")
        arr_int = arr.astype(int, copy=False)
        num_classes = int(arr_int.max()) + 1 if arr_int.size else 1
        encoded[task] = np.eye(num_classes, dtype=np.float32)[arr_int]
        meta[task] = arr.dtype
    return encoded, meta


def _decode_multi_labels(labels: Dict[str, np.ndarray], meta: Dict[str, np.dtype]) -> Dict[str, np.ndarray]:
    return {
        task: np.argmax(encoded, axis=1).astype(meta.get(task, np.int64), copy=False)
        for task, encoded in labels.items()
    }
...
def balance_with_mt_wgan_gp(
    ...
    if save_path:
        ...
        if gan is not None and loaded_meta is not None:
            if seq_len_value is None:
                seq_len_value = loaded_meta["seq_len"]
            if (
                loaded_meta["seq_len"] == seq_len_value
                and loaded_meta["num_features"] == num_features
                and loaded_meta["task_label_dims"] == task_label_dims
            ):
                if verbose:
                    print(f"    Using existing MT WGAN-GP model from {save_path}")
            else:
                if verbose:
    ...
    if not model_was_loaded:
        ...
        if save_path:
            metadata = {
                ...
                "task_loss_weights": dict(gan.task_loss_weights),  # Convert to plain dict to avoid pickle issues
            }
    ...
    aug_x = aug_x[perm]
    aug_y = {task: labels[perm] for task, labels in aug_y.items()}
