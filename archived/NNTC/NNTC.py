...
class NNTC(IStrategy):
   ...
   # trailing stoploss
    ...
    use_loss_threshold = CategoricalParameter([True, False], default=False, space='sell', load=True, optimize=False)
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Guard Metric in buy region
        conditions.append(dataframe['guard_metric'] <= -0.5)
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: 'datetime', current_rate: float,
        ...
        if current_profit > 0.005:
            if last_candle['guard_metric'] > 0.8:
                return 'take_profit'

        # Mod: strong sell signal, in profit
        if (current_profit > 0) and (last_candle['guard_metric'] > 0.93):
            return 'metric_high'
