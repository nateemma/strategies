...
class Anomaly(IStrategy):
    ...
    def get_train_buy_signals(self, future_df: DataFrame):
        ...
        if self.training_signals.check_indicators(future_df): # type: ignore
            signals = self.training_signals.get_entry_training_signals(future_df) # type: ignore
        else:
    ...
    def get_train_sell_signals(self, future_df: DataFrame):
        ...
        if self.training_signals.check_indicators(future_df): # type: ignore
            signals = self.training_signals.get_exit_training_signals(future_df) # type: ignore
    ...
    def get_strategy_entry_guard_conditions(self, dataframe: DataFrame):
        return self.training_signals.get_entry_guard_conditions(dataframe) # type: ignore

    def get_strategy_exit_guard_conditions(self, dataframe: DataFrame):
        return self.training_signals.get_exit_guard_conditions(dataframe) # type: ignore
    ...
    def print_strategy_info(self):
        ...
        print(f"    Signal Type:            {self.signal_type} ({self.training_signals.get_signal_name()})") # type: ignore
    ...
    def create_training_data(self, dataframe: DataFrame):
        ...
        future_df = self.dataframePopulator.add_hidden_indicators(dataframe.copy()) # type: ignore
        future_df = self.dataframePopulator.add_future_data(future_df, self.curr_lookahead) # type: ignore
    ...
    def save_debug_indicators(self, future_df: DataFrame):
        dbg_list = self.training_signals.get_debug_indicators() # type: ignore
    ...
    def train_models(self, curr_pair, dataframe: DataFrame, buys, sells) -> DataFrame:
        ...
        full_df_norm = self.dataframeUtils.norm_dataframe(dataframe) # type: ignore
        ...
        df_train, df_test = self.dataframeUtils.split_dataframe(full_df_norm, train_ratio) # type: ignore
        train_buys, test_buys = self.dataframeUtils.split_array(buys, train_ratio) # type: ignore
        train_sells, test_sells = self.dataframeUtils.split_array(sells, train_ratio) # type: ignore
        ...
        train_buy_labels = self.dataframeUtils.get_binary_labels(train_buys) # type: ignore
        train_sell_labels = self.dataframeUtils.get_binary_labels(train_sells) # type: ignore
        test_buy_labels = self.dataframeUtils.get_binary_labels(test_buys) # type: ignore
        test_sell_labels = self.dataframeUtils.get_binary_labels(test_sells) # type: ignore

        # force train/fit the classifiers in backtest mode only
        force_train = True if (self.dp.runmode.value in ('backtest')) else False

        # Buy Classifier
        self.buy_classifier.train(df_train, df_test, train_buys, test_buys, force_train=force_train) # type: ignore

        # Sell Classifier
        if self.enable_exit_signal.value:
            self.sell_classifier.train(df_train, df_test, train_sells, test_sells, force_train=force_train) # type: ignore
        ...
        if self.dp.runmode.value in ('plot'):
            if self.compress_data:
                df_norm = self.dataframeUtils.norm_dataframe(dataframe)  # type: ignore # this also resets the scaler
                df_compressed = self.compress_dataframe(df_norm)
                df_recon_compressed = self.buy_classifier.reconstruct(df_compressed) # type: ignore
                df_recon_norm = self.compressor.inverse_transform(df_recon_compressed) # type: ignore
                df_recon_norm = pd.DataFrame(df_recon_norm, columns=df_norm.columns)
                df_recon = self.dataframeUtils.denorm_dataframe(df_recon_norm) # type: ignore
                dataframe['%recon'] = df_recon['close']
            else:
                # debug: get reconstructed dataframe and save 'close' as a comparison
                tmp = self.dataframeUtils.norm_dataframe(dataframe)  # type: ignore # this just resets the scaler
                df_recon_norm = self.buy_classifier.reconstruct(tmp) # type: ignore
                df_recon = self.dataframeUtils.denorm_dataframe(df_recon_norm) # type: ignore
                dataframe['%recon'] = df_recon['close']
        return dataframe
    ...
    def compress_dataframe(self, df_norm: DataFrame) -> DataFrame:
        ...
        return pd.DataFrame(self.compressor.transform(df_norm)) # type: ignore
    ...
    def predict_buy(self, df: DataFrame, pair):
        ...
        predict = np.where((predict > 0) & (df['mfi'] < 50), 1.0, 0.0) # type: ignore
    ...
    def custom_stoploss(self, pair: str, trade: Trade, current_time: datetime, current_rate: float, # type: ignore
    ...
    def custom_exit(self, pair: str, trade: Trade, current_time: 'datetime', current_rate: float,
        ...
        if (current_profit > 0) and (last_candle['fisher_wr'] >= self.cexit_fwr_overbought.value): # type: ignore
            return 'fwr_overbought'

        # Above 1%, sell if Fisher/Williams in sell range
        if current_profit > 0.01:
            if last_candle['fisher_wr'] >= self.cexit_fwr_take_profit.value: # type: ignore
                return 'take_profit'
        ...
        if (current_profit > 0) and (last_candle['exit_long'] > 0): # type: ignore
            return 'exit_signal'
