...
"""
class FBB_MacheteV8b(IStrategy):


    # Buy hyperspace params:
    buy_params = {
        "buy_bb_gain": 0.06, 
        "buy_enable_adx_momentum": True, 
        ...
        "buy_enable_macd_strategy_crossed": False, 
        "buy_enable_pmax": True, 
        "buy_enable_quickie": False, 
        "buy_enable_scalp": False, 
        "buy_enable_simple": True, 
        "buy_enable_strategy001": False, 
        "buy_enable_technical_example_strategy": False, 
        "buy_enable_tema_rsi_strategy": False, 
        "buy_fisher_wr": -0.62, 
        "buy_force_fisher_wr": -0.75, 
    }
    ...
    sell_params = {
        ...
        "cstp_threshold": -0.007,
        "cstp_trailing_max_stoploss": 0.046,
    ...
    buy_fisher_wr = DecimalParameter(-0.99, -0.5, decimals=2, default=-0.75, space='buy', load=True, optimize=True)
    buy_force_fisher_wr = DecimalParameter(-0.99, -0.75, decimals=2, default=-0.99, space='buy', load=True, optimize=True)

    # MacheteV8b hyperparams
    buy_enable_awesome_macd = CategoricalParameter([True, False], default=buy_params['buy_enable_awesome_macd'], space='buy', optimize=True)
    buy_enable_adx_momentum = CategoricalParameter([True, False], default=buy_params['buy_enable_adx_momentum'], space='buy', optimize=True)
    ...
    buy_enable_macd_strategy = CategoricalParameter([True, False], default=buy_params['buy_enable_macd_strategy'], space='buy', optimize=True)
    buy_enable_pmax = CategoricalParameter([True, False], default=buy_params['buy_enable_pmax'], space='buy', optimize=True)
    buy_enable_quickie = CategoricalParameter([True, False], default=buy_params['buy_enable_quickie'], space='buy', optimize=True)
    buy_enable_scalp = CategoricalParameter([True, False], default=buy_params['buy_enable_scalp'], space='buy', optimize=True)
    buy_enable_simple = CategoricalParameter([True, False], default=buy_params['buy_enable_simple'], space='buy', optimize=True)
    buy_enable_strategy001 = CategoricalParameter([True, False], default=buy_params['buy_enable_strategy001'], space='buy', optimize=True)
    buy_enable_technical_example_strategy = CategoricalParameter([True, False], default=buy_params['buy_enable_technical_example_strategy'], space='buy', optimize=True)
    buy_enable_tema_rsi_strategy = CategoricalParameter([True, False], default=buy_params['buy_enable_tema_rsi_strategy'], space='buy', optimize=True)
    ...
    cstp_bb_trailing_input = CategoricalParameter(['bb_lowerband_trend_inf', 'bb_lowerband_neutral_inf', 'bb_upperband_neutral_inf'], default=sell_params['cstp_bb_trailing_input'], space='sell', optimize=True)
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative1 = self.get_market_condition_indicators(informative_tmp.copy(), metadata)
        informative = self.get_custom_stoploss_indicators(informative1.copy(), metadata)
    ...
    def get_buy_signal_scalp(self, dataframe: DataFrame):
        ...
        dataframe.loc[signal, 'buy_tag'] += 'scalp '
        return signal

    def get_buy_signal_simple(self, dataframe: DataFrame):
        signal = (
            (self.buy_enable_simple.value == True) &
        ...
        dataframe.loc[signal, 'buy_tag'] += 'simple '
        return signal

    def get_buy_signal_strategy001(self, dataframe: DataFrame):
        signal = (
            (self.buy_enable_strategy001.value == True) &
            qtpylib.crossed_above(dataframe['ema50'], dataframe['ema100']) &
            (dataframe['ha_close'] < dataframe['ema20']) &
            (dataframe['ha_open'] > dataframe['ha_close'])  # red bar
        )
        dataframe.loc[signal, 'buy_tag'] += 's1 '
        return signal

    def get_buy_signal_technical_example_strategy(self, dataframe: DataFrame):
        signal = (
            (self.buy_enable_technical_example_strategy.value == True) &
            (dataframe['cmf'] < 0)
        )
        dataframe.loc[signal, 'buy_tag'] += 'tech '
        return signal

    def get_buy_signal_tema_rsi_strategy(self, dataframe: DataFrame):
        signal = (
            (self.buy_enable_tema_rsi_strategy.value == True) &
            (qtpylib.crossed_above(dataframe['rsi'], 30)) &  # Signal: RSI crosses above 30
            (dataframe['tema'] <= dataframe['bb_middleband']) &  # Guard: tema below BB middle
            (dataframe['tema'] > dataframe['tema'].shift(1))
        )
        dataframe.loc[signal, 'buy_tag'] += 'tema '
        return signal
