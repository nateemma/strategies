...
"""
####################################################################################
Kalman2 - use a Kalmqn Filter to estimate future price movements,
          but *without* Fisher/Williams/Bollinger buy/sell signals
          The Kalman filter is good at detecting swings, while the FBB checks are to try and keep
...
class Kalman2b(IStrategy):
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        kf2_cond = (
                (qtpylib.crossed_above(dataframe['kf_dev_diff'], 5.0))
        )

        conditions.append(kf_cond | kf2_cond)

        # set buy tags
        dataframe.loc[kf_cond, 'buy_tag'] += 'kf_buy_1 '
        dataframe.loc[kf2_cond, 'buy_tag'] += 'kf_buy_2 '
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        kf2_cond = (
            (qtpylib.crossed_below(dataframe['kf_dev_diff'], -5.0))
        )
        ...
        dataframe.loc[kf2_cond, 'exit_tag'] += 'kf_sell_2 '
