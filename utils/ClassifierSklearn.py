...
class ClassifierSklearn():
    ...
    batch_size = None  # For compatibility with ClassifierKeras API (not used by sklearn)
    ...
    def train(self, df_train_norm: DataFrame, df_test_norm: DataFrame, train_labels, test_labels, force_train=False, class_weights=None):
    ...
    def predict(self, df_norm: DataFrame):
        ...
        print("    ERR: classifier does not have a predict(), predict_proba(), or fit_predict() method")
        n_samples = np.shape(df_norm)[0]
