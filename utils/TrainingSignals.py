# pylint: disable=C0103,C0114,C0301,C0116,C0413,C0115,W0611,W0613,UP034
...
class base_signals(ABC):
    ...
    def indicators_present(self, ind_list, dataframe) -> bool:
        if len(ind_list) > 0:
            ...
            print("    WARNING: empty indicator list provided")
            result = False
    ...
    # utility method for smoothing a series
    def smooth(self, series, window):
...
#   fairly simple oversold/overbought signals (with future profit/loss)
...
# gmbb = guard_metric and Bollinger Band
# guard_metric is (currently) RMI, scaled to [-1, 1]
#   -ve values indicate an oversold region, +ve overbought
# bb_gain and bb_loss represent the potential gain/loss if the price jumps to top/bottom of the current
#    Bollinger Band
...
class gmbb_signals(base_signals):

    def check_indicators(self, dataframe) -> bool:
        # check that needed indicators are present
        ind_list = [
             'guard_metric', 'bb_gain',
            'profit_threshold', 'loss_threshold',
            'future_loss_min', 'future_loss_threshold',
            'future_profit_max', 'future_profit_threshold'
        ]
...
# lpv - 'Large' peak/valley detection

class lpv_signals(base_signals):
    ...
    def get_entry_training_signals(self, future_df: DataFrame):
        ...
        # get smoothed version of close signal
        close = self.smooth(future_df['close'], 12)

        # get valleys
        valleys = np.zeros(future_df.shape[0], dtype=float)
        order = 4

        v_idx = scipy.signal.argrelextrema(close, np.less_equal, order=order)[0]
        valleys[v_idx] = 1

        # # identify regions below running mean
        # rmean = future_df['close'].rolling(window=12).mean()
        # # rmean = ta.TEMA(future_df, timeperiod=24)
        # std = future_df['close'].rolling(window=12).std()
        # lows = np.where((future_df['close'] < (rmean - 2.0 * std)), 1, 0)
        lows = np.where((future_df['close'] <= future_df['bb_lowerband']), 1, 0)

        # print(f'future_df: {future_df.shape} valleys: {np.shape(valleys)}')
        signals = np.where(
            (
                # valley and in low region
                    (valleys > 0) & (lows > 0) #&

                    # # future profit
                    # (future_df['future_profit_max'] >= future_df['future_profit_threshold']) &
                    # (future_df['future_profit_max'] > 0) #&
                    # # (future_df['future_gain'] > 0)
            ), 1.0, 0.0)
        return signals
    ...
    def get_exit_training_signals(self, future_df: DataFrame):
        ...
        p_idx = scipy.signal.argrelextrema(close, np.greater_equal, order=order)[0]
        peaks[p_idx] = 1.0

        # identify regions above running mean
        # rmean = future_df['close'].rolling(window=12).mean()
        # # rmean = ta.TEMA(future_df, timeperiod=24)
        # std = future_df['close'].rolling(window=12).std()
        # highs = np.where((future_df['close'] > (rmean + 2.0 * std)), 1, 0)
        highs = np.where((future_df['close'] >= future_df['bb_upperband']), 1, 0)

        signals = np.where(
            (
                    # peak and in high region
                    (peaks > 0) & (highs > 0) #&

                    # future loss
                    # (future_df['future_loss_min'] <= future_df['future_loss_threshold'])&
                    # (future_df['future_loss_min'] < 0) #&
                    # (future_df['future_gain'] < 0)
            ), 1.0, 0.0)
        return signals
...
class SignalType(Enum):
    ...
    Guard_Bollinger = gmbb_signals
    High_Low = highlow_signals
    Jump = jump_signals
    Large_Peaks_Valleys = lpv_signals
    MACD = macd_signals
    MACD2 = macd2_signals
    MACD3 = macd3_signals
