...
def breakouts(df: DataFrame, length=20):
    ...
    df["resistance_level"] = df["resistance_level"].combine_first(
        df["resistance_level"].shift()
    )
...
def exhaustion_bars(
    dataframe, maj_qual=6, maj_len=12, min_qual=6, min_len=12, core_length=4
):
    """
...
def dynamic_exhaustion_bars(dataframe, window=500):
    ...
    dataframe["pct_change_zscore_smoothed"] = (
        dataframe["pct_change_zscore"].rolling(window=3).mean().fillna(1.0)
    )
    ...
    dataframe = populate_leledc_major_minor(
        dataframe, maj_qual, min_qual, maj_len, min_len
    )
