...
class _DFGeneratorCNN(Model):
    def __init__(
        ...
        # Start from a compact temporal size
        start_len = max(1, seq_len // (2 ** num_upsample_blocks))
        channels = base_filters
        x = layers.Dense(start_len * channels, activation=None)(x)
        x = layers.LeakyReLU(negative_slope=0.2)(x)
        x = layers.Reshape((start_len, channels))(x)

        def film_block(y, cond, channels):
            gamma_beta = layers.Dense(2 * channels, activation=None)(cond)
            gamma, beta = layers.Lambda(lambda t: tf.split(t, num_or_size_splits=2, axis=-1))(gamma_beta)
            gamma = layers.Reshape((1, channels))(gamma)
            beta = layers.Reshape((1, channels))(beta)
        ...
        for _ in range(num_upsample_blocks):
            ...
            # Optional reduce channels toward output
        ...
        # If temporal length mismatch, resize to exact seq_len
        def resize_to_len(t):
            cur_len = tf.shape(t)[1]
            def resized():
                # Use 2D resize by adding a width dim of 1
                tt = tf.expand_dims(t, axis=2)  # (B, L, 1, C)
                tt = tf.image.resize(tt, size=(self.seq_len, 1), method="bilinear")
                tt = tf.squeeze(tt, axis=2)
                return tf.cast(tt, t.dtype)
            same = lambda: t
            return tf.cond(tf.equal(cur_len, self.seq_len), same, resized)
