...
from CustomLoss import focal_loss_sigmoid, focal_loss_softmax, dice_loss
from CustomMetric import f1_micro_metric
...
class ClassifierKerasBinary(ClassifierKeras):
    ...
    def create_model(self, seq_len, num_features):
        ...
        model.add(layers.LSTM(256, return_sequences=True, activation='tanh', input_shape=(seq_len, num_features)))
        ...
        model.add(layers.Dense(1, activation='sigmoid', 
                              ...
                              bias_initializer='zeros'))  # Neutral bias since we're using class weights
    ...
    def load(self, path=""):
        ...
        if model is not None:
            try:
                ...
                print(f"      Model name: {model.name if hasattr(model, 'name') else 'unknown'}")
                ...
                if hasattr(model, 'optimal_threshold'):
                    ...
                    print(f"    Loaded optimal threshold from model: {self.optimal_threshold:.3f}")
                else:
                    # Use default threshold for imbalanced data
                    self.optimal_threshold = 0.25  # Default for imbalanced data
                    print(f"    Using default optimal threshold: {self.optimal_threshold:.3f}")
                    print(f"    Note: For best results, retrain model to calculate data-specific threshold")
    ...
    def compile_model(self, model):
        ...
        if self.use_focal_loss:
            ...
            print(f"    Using lower learning rate ({learning_rate}) for BCE to prevent training collapse")
        ...
        if output_shape == 1:
            ...
            if self.use_focal_loss:
                model.compile(optimizer=optimizer, loss=focal_loss_sigmoid,
            ...
            if self.use_focal_loss:
                model.compile(optimizer=optimizer, loss=focal_loss_softmax,
                              metrics=['accuracy', 'precision', 'recall', f1_micro_metric])
                print("    🎯 Using focal loss (better for imbalanced data)")
            else:
    ...
    def train(self, df_train_norm, df_test_norm, train_results, test_results, force_train=False):
        ...
        if self.dataframeUtils.is_dataframe(df_train_norm):
            ...
            train_tensor = self.dataframeUtils.df_to_tensor(df_train, self.seq_len, method=1)
            test_tensor = self.dataframeUtils.df_to_tensor(df_test, self.seq_len, method=1)
        else:
        ...
        class PredictionVarianceCallback(tf.keras.callbacks.Callback):
            ...
            def on_epoch_end(self, epoch, logs=None):
                if epoch % 3 == 0:  # Check more frequently
                    ...
                    sample_preds = self.model.predict(train_tensor[:sample_size], verbose=0)
                    # Normalize raw predictions to 1D
                    try:
                        # Use the normalize method from the classifier instance
                        sample_preds = self.classifier._normalize_raw_predictions(sample_preds)
                    except Exception as e:
                    ...
                    print(f"      Epoch {epoch}: pred_mean={pred_mean:.4f}, pred_std={pred_std:.6f}, range=[{pred_min:.4f}, {pred_max:.4f}]")
                    print(f"      Zero predictions (<{zero_threshold}): {zero_count}/{len(sample_preds)} ({zero_percentage:.1f}%)")

                    # Detect all-zero predictions
                    if pred_std < 1e-4 or zero_percentage > 95:
                        self.zero_prediction_count += 1
                        print(f"      ⚠️  WARNING: Low prediction variance or all-zero predictions at epoch {epoch}")
                        print(f"      🔍 DEBUG: All predictions are {pred_mean:.4f}")
                        print(f"      🚨 Zero prediction count: {self.zero_prediction_count}")
                        ...
                        if hasattr(self.model, 'layers'):
                            for i, layer in enumerate(self.model.layers):
                                if hasattr(layer, 'kernel') and layer.kernel is not None:
                                    ...
                                    print(f"      Layer {i} ({layer.name}) kernel norm: {grad_norm:.6f}")
                    ...
                    if logs and 'f1_micro_metric' in logs:
                        ...
                        if f1_score < 0.01:  # Very low F1 score
                            ...
                            print(f"      ⚠️  WARNING: Low F1 score detected: {f1_score:.6f} (epoch {epoch})")
                        else:
                            self.low_f1_epochs = 0

                    # Suggest fallback to BCE if focal loss is failing
                    if self.low_f1_epochs >= 3 and not self.fallback_to_bce:
                        print(f"      💡 SUGGESTION: Consider switching to BCE loss (low F1 for {self.low_f1_epochs} epochs)")
                        print(f"      🔧 To fix: Set use_focal_loss = False in ClassifierKerasBinary")
                        self.fallback_to_bce = True

                    # Early stopping if model is completely dead
                    if (pred_std < 1e-6 or zero_percentage > 99) and epoch > 3:
                        print(f"      🛑 STOPPING: Model is completely dead at epoch {epoch}")
                        print(f"      💡 Suggestion: Try reducing gamma, increasing alpha, or using BCE loss")
                        self.model.stop_training = True
                    
                    # Also stop if F1 score is consistently very low
                    if logs and 'f1_micro_metric' in logs:
                        f1_score = logs['f1_micro_metric']
                        if f1_score < 0.001 and epoch > 5:  # Very low F1 score for multiple epochs
                            print(f"      🛑 STOPPING: Consistently low F1 score ({f1_score:.6f}) at epoch {epoch}")
                            print(f"      💡 Suggestion: Check class balance and consider focal loss")
                            self.model.stop_training = True
        ...
        if hasattr(self, 'original_train_class_indices') and self.original_train_class_indices is not None:
            ...
            if self.use_focal_loss:
                ...
                class_weights_arr = compute_class_weight('balanced', classes=unique, y=train_class_indices)
                ...
                max_weight = min(max(class_weights_arr)/2.0, 30.0)  # Cap to prevent overfitting
                min_weight = min(class_weights_arr)  # Lower weights for majority class 

                if max_weight is not None:
                    class_weights_arr = np.clip(class_weights_arr, min_weight, max_weight)
                    print(
                        f"    Class weights capped: min={min_weight}, max={max_weight}"
                    )
                else:
                    print(f"    Class weights uncapped (raw sklearn values)")
            ...
            print("train_results unique/counts:", np.unique(train_results, return_counts=True))
            if len(unique) == 2:
                # Handle one-hot encoded labels
                if hasattr(train_results, 'ndim') and train_results.ndim > 1 and train_results.shape[1] == 2:
                    labels_flat = np.argmax(train_results, axis=1)
                    print("Detected one-hot encoded labels. Converted to class indices for class weight calculation.")
                else:
                    labels_flat = np.asarray(train_results).ravel()
                class_weights_arr = compute_class_weight('balanced', classes=unique, y=labels_flat)
                class_weights = dict(zip(unique, class_weights_arr))
                print(f"Class weights (sklearn): {class_weights}")
                print(f"Class distribution: {dict(zip(unique, counts))}")

        callbacks = [plateau_callback, early_callback, checkpoint_callback, variance_callback]
        ...
        if hasattr(fhis, 'history'):
            ...
            if 'accuracy' in fhis.history:
                print(f"    Final training accuracy: {fhis.history['accuracy'][-1]:.4f}")
                print(f"    Final validation accuracy: {fhis.history['val_accuracy'][-1]:.4f}")
            ...
            print(f"    Training predictions: {np.unique(train_preds, return_counts=True)}")
            ...
            optimal_predictions = self.predict_with_threshold(test_tensor, optimal_threshold)
            print(f"    Optimal predictions - unique/counts: {np.unique(optimal_predictions, return_counts=True)}")
    ...
    def analyze_raw_predictions(self, raw_predictions):
        ...
        print(f"      Non-zero count: {non_zero_count} ({non_zero_count/total_count*100:.1f}%)")
    ...
    def suggest_threshold(self, raw_predictions):
        ...
        suggested_threshold = clipped_mean + 0.10  # Add larger bias to reduce false positives

        # Clamp to reasonable range
        suggested_threshold = np.clip(suggested_threshold, 0.1, 0.9)

        print(f"    💡 Suggested threshold: {suggested_threshold:.3f} (clipped-mean-based)")
        print(f"      Clipped mean (0.1-0.9): {clipped_mean:.3f}")
        print(f"      Non-zero mean: {analysis['non_zero_mean']:.3f}, Std: {analysis['non_zero_std']:.3f}")
    ...
    def calculate_dynamic_threshold(self, raw_predictions):
        ...
        print(f"      Distribution: mean={mean_val:.3f}, std={std_val:.3f}, median={median_val:.3f}")
        print(f"      Clipped mean (0.1-0.9): {clipped_mean:.3f}")

        return threshold

    def find_optimal_threshold(self, validation_data, validation_labels, thresholds=None):
        """
        ...
        if self.dataframeUtils.is_dataframe(validation_data):
            val_tensor = self.dataframeUtils.df_to_tensor(validation_data, self.seq_len, method=1)
        else:
        ...
        print(f"    Best recall threshold: {best_recall_threshold:.3f} (Recall: {best_recall:.3f})")
        print(f"    Note: Use recall threshold if you want to catch more buy opportunities")

        # Also suggest threshold based on non-zero statistics
        suggested_threshold = self.suggest_threshold(raw_preds)
        print(f"    Suggested threshold: {suggested_threshold:.3f} (based on non-zero statistics)")
    ...
    def tune_threshold(self, validation_data, validation_labels, method='f1'):
        ...
        print(f"    F1: {best_result['f1']:.3f}, Precision: {best_result['precision']:.3f}, Recall: {best_result['recall']:.3f}")

        return best_result['threshold']

    def evaluate_model_quality(self, train_buy, predict_buy, threshold=0.5, verbose=True):
        """
        ...
        if predict_buy.dtype == float and np.max(predict_buy) <= 1.0 and np.min(predict_buy) >= 0.0:
        ...
        class_imbalance_ratio = negative_samples / positive_samples if positive_samples > 0 else float('inf')
        
        # Prediction distribution analysis
        prediction_imbalance_ratio = negative_predictions / positive_predictions if positive_predictions > 0 else float('inf')
        ...
        if verbose:
            ...
            print(f"      Negative predictions: {negative_predictions} ({negative_predictions/total_samples*100:.1f}%)")
            print(f"      Class imbalance ratio: {class_imbalance_ratio:.2f}:1")
            print(f"      Prediction imbalance ratio: {prediction_imbalance_ratio:.2f}:1")
            ...
            # Specific issues
            if precision < 0.3:
                print(f"      ⚠️  Low Precision: Model has many false positives")
            if recall < 0.3:
                print(f"      ⚠️  Low Recall: Model misses many positive cases")
            if abs(class_imbalance_ratio - prediction_imbalance_ratio) > 2:
                print(f"      ⚠️  Prediction distribution doesn't match class distribution")
            if mcc < 0.1:
                print(f"      ⚠️  Low MCC: Model shows poor correlation with true labels")
            
            print(f"    {'='*50}")
        
        return results

    def compare_train_predict(self, train_data, train_labels, predict_data, predict_labels, 
        ...
        # Evaluate both sets
        train_results = self.evaluate_model_quality(train_labels, train_preds_binary, verbose=False)
        predict_results = self.evaluate_model_quality(predict_labels, predict_preds_binary, verbose=False)
        ...
        for metric in metrics:
            ...
            comparison['differences'][metric] = {
                ...
                'predict': predict_val,
                'difference': diff,
                'percent_change': (diff / train_val * 100) if train_val != 0 else float('inf')
            }
        
        # Print comparison
        print(f"\n    🔄 TRAIN vs PREDICT COMPARISON:")
        print(f"    {'='*60}")
        print(f"    {'Metric':<15} {'Train':<10} {'Predict':<10} {'Diff':<10} {'% Change':<10}")
        print(f"    {'-'*60}")
        
        for metric in metrics:
            diff_data = comparison['differences'][metric]
            print(f"    {metric:<15} {diff_data['train']:<10.4f} {diff_data['predict']:<10.4f} "
        ...
        if abs(avg_f1_diff) < 0.05:
            ...
            print(f"      ⚠️  Model may be overfitting (F1 difference: {avg_f1_diff:.4f})")
        else:
            print(f"      ⚠️  Model may be underfitting (F1 difference: {avg_f1_diff:.4f})")
