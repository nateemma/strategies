...
class ema_detrender(base_detrender):
    ...
    def detrend_1d(self, data: np.array) -> np.array:
        ...
        x_detrend[0] = data[0]  # Start EMA from the first data point

        for t in range(1, len(data)):
            x_detrend[t] = alpha * data[t] + (1 - alpha) * x_detrend[t - 1]  # EMA calculation

        self.poly = data - x_detrend
        return x_detrend

    def retrend_1d(self, data: np.array) -> np.array:
        # polynomial can be different length because training data can be different than prediction data. So, use last portion
...
# use  Moving Average
class ma_detrender(base_detrender):

    x_orig = 0.0

    def detrend_1d(self, data: np.array) -> np.array:
        x_detrend = np.zeros_like(data)
        self.poly = data
        self.x_orig = data[0]
        window_size = 4


        # Create a moving average using numpy's convolve function
        weights = np.ones(window_size) / window_size
        x_detrend = np.convolve(data, weights, mode='same')
...
class dwt_detrender(base_detrender):
    ...
    def detrend_1d(self, data: np.array) -> np.array:
        ...
        threshold = 0.8 # adjust this value to control the amount of denoising
