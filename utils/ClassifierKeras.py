...
class ClassifierKeras(ClassifierBase):
    ...
    def __init__(self, pair, seq_len, num_features, tag=""):
        ...
        # self.last_time_step_layer = keras.layers.Lambda(self.last_time_step)
    ...
    def set_batch_size(self, batch_size):
        ...
        # print(f"    set_batch_size: {self.batch_size}")
        return
        ...
        if os.path.exists(path):
            ...
            try:
                ...
                if callable(custom_load):
                    # model = self.custom_load(path, custom_objects={'last_time_step': self.last_time_step})
    ...
    def get_latest(self, x):
        ...
        if x.ndim == 3:
            # x = self.last_time_step_layer(x)
            # x =  tf.keras.layers.Lambda(self.last_time_step, output_shape=(x.shape[-1],))(x) # type: ignore
            ...
            if input_shape[1] == 1:
                ...
                # Create indices for the last element of each sequence
                indices = input_shape[1] - 1

                # Use Keras ops to select the last element
                x = keras.ops.take(x, indices, axis=1)

        return x

    # get the earliest sequence and collapse to 2D (suitable for input to a Dense layer)

    def get_earliest(self, x):
        ...
        if x.ndim == 3:
            ...
            if input_shape[1] == 1:
                ...
                # Use Keras ops to select the first element
                x = keras.ops.take(x, 0, axis=1)
