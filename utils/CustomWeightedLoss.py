...
class CustomWeightedLoss(tf.keras.losses.Loss):
    ...
    def f1_weighted_loss(self, y_true, y_pred):
        ...
        weighted_f1 = weighted_f1 * self.class_weights

        weighted_f1 = tf.keras.backend.sum(weighted_f1)
        # weighted_f1 = tf.keras.backend.mean(weighted_f1)

        max_f1 = tf.keras.backend.sum(tf.keras.backend.cast(self.class_weights, 'float'), axis=0)
        f1_loss = tf.where(tf.math.is_nan(weighted_f1), max_f1, 1.0 - weighted_f1 / max_f1)
        return f1_loss
    ...
    def weighted_precision_loss(self, y_true, y_pred):
        ...
        # p_mean = tf.keras.backend.mean(w_p)
        p_mean = tf.keras.backend.sum(w_p)
        
        max_p = tf.keras.backend.sum(tf.keras.backend.cast(self.class_weights, 'float'), axis=0)
        return 1.0 - p_mean/max_p
    ...
    def weighted_categorical_loss(self, y_true, y_pred):
        ...
        return tf.keras.backend.mean(weighted_ce_loss)  # or sum
    ...
    def call(self, y_true, y_pred):
        ...
        elif self.loss_type == CustomWeightedLoss.WeightedLossType.F1_WEIGHTED:
            return self.f1_weighted_loss(y_true, y_pred)
        elif self.loss_type == CustomWeightedLoss.WeightedLossType.F1_BETA:
            return self.f1_beta_loss(y_true, y_pred)
        elif self.loss_type == CustomWeightedLoss.WeightedLossType.PRECISION_WEIGHTED:
            return self.weighted_precision_loss(y_true, y_pred)
        else:
