...
from CustomMetric import MinorityF1, Class2F1, MCCMetric, AverageMCCCallback
...
class ClassifierKerasMultiTask(ClassifierKeras):
    """
    ...
    def get_initial_pca_matrices(
        ...
        print(f"    Pre-loading PCA estimate")
    ...
    def _check_training_results(self, history):
        ...
        if epochs_trained < 10:
            ...
            print(
                f"    Consider increasing early stopping patience or adjusting learning parameters"
            )
        ...
        if hasattr(self, "early_callback") and hasattr(
            ...
            if monitor_field not in history.history:
                ...
                if val_metrics:
                    ...
                    print(
                        f"    Cannot determine best epoch - no validation metrics found"
                    )
                    return
        ...
        if val_trading_f1_key in history.history:
            ...
            if best_val_trading_f1 < 0.2:
                ...
                print(
                    f"    Model may not be performing well for trading classification"
                )
            else:
                print(
            ...
            print(
                f"    WARNING: Could not find validation trading F1 score in training history"
            )
            print(f"    Available metrics: {list(history.history.keys())}")
    ...
    def _calculate_recommended_weights(self, history, best_epoch_idx):
        ...
        if len(task_losses) < 2:
            print(
                f"    WARNING: Not enough task losses found to calculate recommended weights"
            )
            return
        ...
        print(f"\n    Option 2: PRIORITIZE HIGH LOSS - Focus on worst-performing tasks")
        print(f"    (High loss tasks get higher weights to improve them faster)")
        print(f"    ")
        print(f"    loss_weights = {{")
