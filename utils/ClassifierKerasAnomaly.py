...
# Inherits from ClassifierKeras to follow the existing pattern
...
from CustomLoss import multi_class_focal_loss
from CustomMetric import MinorityF1, Class2F1, MCCMetric
...
class ClassifierKerasAnomaly(ClassifierKeras):
    """
    ...
    def train(self, df_train_norm, df_test_norm, train_results, test_results, force_train=False, class_weights=None):
        ...
        if self.model is not None:
            ...
            if set(current_outputs) != set(expected_outputs):
                print(f"    Model architecture changed: expected {expected_outputs}, got {current_outputs}")
        ...
        if (self.model is not None) and self.model_is_trained() and (not force_train) and (
                not self.new_model_created()) and (not architecture_changed):
        ...
        print("")
        print(f"    training multi-task model: {self.name}  batch_size:{self.batch_size}")
    ...
    def _check_training_results(self, history):
        ...
        if epochs_trained < 10:
            print(f"    ⚠️  WARNING: Training stopped early after only {epochs_trained} epochs (< 10)")
            print(f"    Consider increasing early stopping patience or adjusting learning parameters")
        ...
        if hasattr(self, 'early_callback') and hasattr(self.early_callback, 'best_epoch'):
            best_epoch_idx = self.early_callback.best_epoch
            if best_epoch_idx is None:
                # Training didn't stop early, use the last epoch
                best_epoch_idx = len(history.history[list(history.history.keys())[0]]) - 1
        else:
            # Fallback: calculate best epoch from history
            if monitor_field not in history.history:
                print(f"    WARNING: Monitor field '{monitor_field}' not found in training history")
                print(f"    Available metrics: {list(history.history.keys())}")
                val_metrics = [k for k in history.history.keys() if k.startswith('val_')]
                if val_metrics:
                    monitor_field = val_metrics[0]
                    print(f"    Falling back to '{monitor_field}' for best epoch determination")
                else:
                    print(f"    Cannot determine best epoch - no validation metrics found")
                    return
        ...
        if monitor_field in history.history:
            ...
            print(f"    Best epoch: {best_epoch_idx + 1} (based on {monitor_field}={best_monitor_value:.4f})")
        else:
            print(f"    Best epoch: {best_epoch_idx + 1}")

        if best_epoch_idx < 10:
            print(f"    ⚠️  WARNING: Training stopped early after only {best_epoch_idx + 1} epochs (< 10)")
            print(f"    Consider increasing early stopping patience or adjusting learning parameters")
        ...
        if val_trading_f1_key in history.history:
            ...
            print(f"    Best validation trading F1 score: {best_val_trading_f1:.4f} (epoch {best_epoch_idx + 1})")

            if best_val_trading_f1 < 0.2:
                print(f"    ⚠️  WARNING: Very Low validation trading F1 score: {best_val_trading_f1:.4f} (< 0.2)")
                print(f"    Model is essentially random guessing")
            elif best_val_trading_f1 < 0.3:
                print(f"    ⚠️  WARNING: Low validation trading F1 score: {best_val_trading_f1:.4f} (< 0.3)")
                print(f"    Model may not be performing well for trading classification")
            else:
                print(f"    ✓ Validation trading F1 score is good: {best_val_trading_f1:.4f}")
        else:
            print(f"    WARNING: Could not find validation trading F1 score in training history")
            print(f"    Available metrics: {list(history.history.keys())}")
