...
from NNPredict.NNPredictor_LSTM import NNPredictor_LSTM

from TS_Simple import TS_Simple

"""
####################################################################################
TS_Simple_LSTM - subclass of TS_Simple that uses an LSTM model

####################################################################################
"""


class TS_Simple_LSTM(TS_Simple):
    ...
    def create_model(self, df_shape):
        ...
        print("    creating LSTM")
        self.model = NNPredictor_LSTM(pair="", seq_len=self.seq_len, num_features=df_shape[1])
