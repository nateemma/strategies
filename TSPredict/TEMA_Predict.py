...
"""
####################################################################################
TEMA_Predict - base class for time series prediction
...
class TEMA_Predict(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe['model_predict'] = dataframe['tema']
    ...
    def add_coefficients(self, dataframe) -> DataFrame:
        ...
        close_data = np.array(dataframe['tema'])
    ...
    def predict(self, col) -> float:
        ...
        close_data = np.array(df['tema'])
        coeff_table = self.get_coefficient_table(close_data)
        df_merged = self.merge_coeff_table(coeff_table, df)
    ...
    def add_rolling_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        close_data = np.array(dataframe['tema']).copy()
    ...
    def add_predictions(self, dataframe: DataFrame) -> DataFrame:
        ...
        if self.dp.runmode.value in ('hyperopt' 'backtest' 'plot'):
            ...
            dataframe['model_predict'] = dataframe['tema'].rolling(window=self.model_window).apply(self.predict, raw=False)
        else:
