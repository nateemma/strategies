...
"""
####################################################################################
DWT_short - use a Discreet Wavelet Transform to estimate future price movements
            Short version (i.e. enters short positions, not long)
...
class DWT_short(IStrategy):
    ...
    # DWT  hyperparams
    # Note that these are the opposite of the 'normal' strat because we are shorting
    entry_dwt_diff = DecimalParameter(-5.0, 0.0, decimals=1, default=1.0, space='buy', load=True, optimize=True)
    exit_dwt_diff = DecimalParameter(0.0, 5.0, decimals=1, default=-1.0, space='sell', load=True, optimize=True)
