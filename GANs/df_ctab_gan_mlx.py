...
class CTABGANMLX:
    ...
    def __init__(self, latent_dim: int = 128, epochs: int = 100, batch_size: int = 512,
                 n_critic: int = 5, verbose: bool = True):
    ...
    def fit(self, dataframe: pd.DataFrame, labels: np.ndarray, categorical_columns: List[str] = [], validation_split: float = 0.1):
        ...
        best_epoch = -1
        checkpoint_dir = tempfile.mkdtemp(prefix="ctab_mlx_ckpt_")
        ...
        for epoch in range(self.epochs):
            ...
            if self.verbose and epoch % 10 == 0:
                print(f"    Epoch {epoch}/{self.epochs} | Critic Loss: {d_val:.4f} | Gen Loss: {g_val:.4f}")
            ...
            if epoch_score < best_score:
                ...
                diverge_count = 0
                try:
                    self.gen.save_weights(best_gen_path)
        ...
        if best_epoch >= 0 and (stopped_early or best_epoch < last_epoch - 1):
            try:
                self.gen.load_weights(best_gen_path)
