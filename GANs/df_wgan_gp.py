...
from GANs.wgan_layers import _SplitLayer, _ResizeToLenLayer, _MinibatchStdLayer
...
class _GenMLP(Model):
    ...
    def __init__(
        ...
        # MLP layers.
        # LayerNorm (not BatchNorm): normalises within each sample so batch-level
        # class-specific mean shifts are preserved.  BatchNorm cancels those shifts,
        # which makes class conditioning impossible.
        # LeakyReLU (not ReLU): allows negative pre-activations so the full [-1,1]
        # tanh range is reachable for all classes.
...
class WGAN_GP(Model):
    def __init__(
        ...
        # class_means: (num_classes, num_features) array of per-class feature means in
        # real-data space. When set, the generator receives an auxiliary loss that penalises
        # generated samples for having the wrong per-class mean, enforcing conditioning.
        self.class_means = (
...
def balance_with_wgan_gp(
    ...
    cond_loss_weight: float = 2.0,  # Weight for class mean matching loss (higher = stronger conditioning)
    ...
    # Compute per-class feature means in real space (shape: num_classes × num_features).
    # Used by the class mean matching loss to enforce conditioning.
    class_idx_all = np.argmax(labels_f32, axis=1)
    class_means_arr = np.zeros((num_classes, num_features), dtype=np.float32)
    for ci in range(num_classes):
        mask = class_idx_all == ci
        if mask.any():
            # data_f32 is (batch, seq_len, num_features); average over seq_len then batch
